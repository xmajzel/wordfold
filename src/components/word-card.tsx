  ...
  return (
    <Animated.View entering={animateEntrance ? FadeInDown.springify().damping(18).reduceMotion(ReduceMotion.System) : undefined} style={[styles.card, dense && styles.denseCard, { backgroundColor: theme.surface, shadowColor: theme.shadow }]}>
      ...
      <WordCardContent dense={dense}>
        <View style={styles.wordSection}><AppText variant="display" style={[styles.word, dense && styles.denseWord]}>{word.term}</AppText>{word.partOfSpeech ? <AppText variant="label" style={{ color: theme.accent }}>{word.partOfSpeech}</AppText> : null}{showPronunciation && wordSupportsPronunciation(word) ? <PronunciationControls text={word.term} sourceLanguageCode={word.sourceLanguageCode} locale={word.sourcePronunciationLocale} catalogSenseId={word.catalogSenseId} compact={dense} inlineWhenPaired active={pronunciationActive}/> : null}</View>
