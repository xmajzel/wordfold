...
export function AiButton({ label, onPress, disabled, loading, action = 'generate' }: {
  ...
  const theme = useAppTheme();
  const color = action === 'discard' ? theme.muted : theme.text;
  const content = <View style={[styles.buttonContent, { backgroundColor: theme.aiSurface }]}>
    {loading ? <ActivityIndicator color={color}/> : <>
      <>{action === 'discard' ? <Ionicons accessible={false} name="close-circle-outline" size={22} color={color}/> : <AiIcon name={action === 'accept' ? 'accept' : 'sparkles'}/>}</>
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled: Boolean(disabled || loading), busy: Boolean(loading) }}
    ...
    {action !== 'discard' ? <LinearGradient colors={theme.aiGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.buttonFrame}>{content}</LinearGradient>
      : <View style={[styles.buttonFrame, { backgroundColor: theme.border }]}>{content}</View>}
  </Pressable>;
}
