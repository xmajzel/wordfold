...
type ReviewedLine = ParsedImportLine & { sense: CatalogSense | null; duplicate: boolean; catalogAmbiguous: boolean };
...
export default function ImportScreen() {
  ...
  const existing = useMemo(() => new Set(words
    .filter((word) => (
      word.sourceLanguageCode === activeCourse.sourceLanguageCode
      && word.targetLanguageCode === activeCourse.targetLanguageCode
    ))
    .map((word) => word.normalizedTerm)), [activeCourse, words]);
  const importable = reviewed?.filter((line) => !line.error && !line.duplicate && (line.sense || line.definition)) ?? [];
  ...
  const review = async () => {
    ...
    try {
      ...
      for (const line of parsed) {
        ...
        const senses = line.error || duplicate || line.definition || !activeCourse.capabilities.bundledCatalog
          ? []
          : await findSenses(line.term, activeCourse.id);
        results.push({
          ...line,
          duplicate,
          sense: senses.length === 1 ? senses[0] : null,
          catalogAmbiguous: senses.length > 1,
        });
      }
      setReviewed(results);
  ...
  const importWords = async () => {
    ...
    try {
      await createWords(selectedImportable.map((line) => ({
        ...
        catalogSenseId: line.sense?.id ?? null,
  ...
  return (
    <Screen scroll>
      ...
      <View><AppText variant="heading">One {languageLabel(activeCourse.sourceLanguageCode)} word per line</AppText><AppText style={{ color: theme.muted }}>{activeCourse.capabilities.bundledCatalog
