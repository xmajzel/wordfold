      ...
      <FlatList
          ...
          {libraryView === 'my-words' ? <>
            ...
            <View style={styles.levelGrid}>
              {cefrLevelSummaries.map((item) => <Pressable
                ...
                style={[styles.levelCard, { backgroundColor: theme.surface, borderColor: theme.border, minWidth: Math.min(160 * fontScale, width - spacing.lg * 2), opacity: item.state === 'available' ? 1 : 0.55 }]}>
        ...
        ListFooterComponent={libraryView === 'discover' ? <View style={styles.footer}>
          ...
          {!activeCourse.capabilities.recommendations ? <View style={[styles.panel, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <View style={styles.recommendationHeading}><View style={[styles.recommendationIcon, { backgroundColor: theme.primarySoft }]}><Ionicons name="shield-checkmark-outline" color={theme.primary} size={22}/></View><View style={styles.packText}><AppText variant="label">Spanish A1 catalog is available</AppText><AppText variant="caption" style={{ color: theme.muted }}>A2–C1 are not yet available; C2 is unsupported by the current source. Instituto Cervantes informs the editorial structure only, with no certification or endorsement implied.</AppText></View></View>
