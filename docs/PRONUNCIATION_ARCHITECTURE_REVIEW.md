...
- **[PHASE 5A/5B/5C IMPLEMENTED LOCALLY / NOT DEPLOYED] Phase 5 (private cloud, opt-in):** the
  ...
  nor signed URLs, separate device/cloud controls, and retryable opt-out deletion. The service-only
  Phase 5C cleanup now locally implements bounded Storage-first deletion, cleanup leases, retries,
  and 30-day audit pruning. Legal approval, remote deployment and scheduling, physical-device
  client verification, and production isolation validation remain release gates. See
  `docs/PRONUNCIATION_PHASE_5A_PRIVATE_BACKEND.md` and
  `docs/PRONUNCIATION_PHASE_5B_PRIVATE_CLIENT.md`, plus
